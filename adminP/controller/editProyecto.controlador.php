...
if (!empty($_FILES['imagen']['name'])) {
    ...
    $uploadFilePathImagen = "../../images/proyectos/".$newNameImagen;
    move_uploaded_file($_FILES['imagen']['tmp_name'], $uploadFilePathImagen);

    $imagen = $newNameImagen;
    }
    consultasSQL::UpdateSQL("proyectos",  "imagen='$imagen'",  "id='$id_proyecto'");
}
...
if (!empty($_FILES['plano']['name'])) {
    ...
    $uploadFilePathPlano = "../../images/proyectos/".$newNamePlano;
    move_uploaded_file($_FILES['plano']['tmp_name'], $uploadFilePathPlano);

    $plano = $newNamePlano;
    }

    consultasSQL::UpdateSQL("proyectos",  "plano='$plano'",  "id='$id_proyecto'");
}
...
if (!empty($_FILES['galeria']['name'])) {
    ...
    for ($h=0; $h < $totalFile ; $h++) {
        ...
        $uploadFilePath = "../../images/proyectos/".$newName;
    ...
    consultasSQL::UpdateSQL("proyectos",  "imagenes='$imagenes'",  "id='$id_proyecto'");
...
$detalles_base = json_encode($resultado_base, JSON_UNESCAPED_UNICODE);
    ...
    consultasSQL::UpdateSQL("proyectos", 
    "nombre='$nombre',
        direccion='$direccion', 
